...
rate_power_3gpp_pilots.py) for the four completed detlam (deterministic-
lambda) checkpoints from the 2026-07-29 lambda/over-transmission fix --
aod=0.0 and aod=0.05, with and without fairness=1.5. All nadir, 3GPP Set-1
defaults (30 dBi / 75 W). aod=0.025 variants excluded (diverged to NaN
mid-training).

MMSE and the pre-fix (rollout-lambda) SAC nadir curve are NOT re-simulated:
both are loaded from the existing rate_power_3gpp_pilots.gzip ('mmse_nadir'
and '3gpp_nadir' keys), which was already computed at the identical system
(30 dBi / 75 W / nadir) -- re-running them would just reproduce the same
numbers at MC cost.
"""
import matplotlib
matplotlib.use('Agg')
...
CHECKPOINTS = [
    {
        'key': 'detlam_aod0.0',
        'label': 'SAC detlam aod0.0',
        'training_name': 'EE_dinkelbach_adaptive_detlam512_N16K3_satg30_p75_eta0.6_rawpow',
    },
    {
        'key': 'detlam_aod0.05',
        'label': 'SAC detlam aod0.05',
        'training_name': 'EE_dinkelbach_adaptive_aod0.05_detlam512_N16K3_satg30_p75_eta0.6_rawpow',
    },
    {
        'key': 'detlam_aod0.0_fair1.5',
        'label': 'SAC detlam aod0.0, fair1.5',
        'training_name': 'EE_dinkelbach_adaptive_detlam512_N16K3_satg30_p75_eta0.6_rawpow_fair1.5',
    },
    {
        'key': 'detlam_aod0.05_fair1.5',
        'label': 'SAC detlam aod0.05, fair1.5',
        'training_name': 'EE_dinkelbach_adaptive_aod0.05_detlam512_N16K3_satg30_p75_eta0.6_rawpow_fair1.5',
    },
]
...
if __name__ == '__main__':
    ...
    out_path = Path(cfg0.output_metrics_path, 'EE_detlam_rate_power_sweep')
    out_path.mkdir(parents=True, exist_ok=True)
    gzip_path = Path(out_path, 'rate_power_detlam.gzip')
    ...
    fig.suptitle('Detlam checkpoints (2026-07-29 fix) vs. pre-fix SAC and MMSE, nadir 30 dBi / 75 W', fontsize=12, y=1.02)
    fig.tight_layout(rect=[0, 0, 1, 0.96])

    pdf_path = Path(plot_cfg.plots_parent_path, 'pdf')
    pdf_path.mkdir(parents=True, exist_ok=True)
    out = Path(pdf_path, 'rate_power_detlam.pdf')
