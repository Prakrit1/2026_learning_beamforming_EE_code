"""test script to evaluate learned performance of SAC: the four completed
detlam (deterministic-lambda) checkpoints from the 2026-07-29 lambda/
over-transmission fix (jobs 154760/154762/154783/154785 -- all nadir, 3GPP
Set-1 defaults 30 dBi / 75 W). The two aod=0.025 variants (154761/154784)
diverged to NaN mid-training and are deliberately skipped here.

MMSE reference is NOT re-run: it doesn't depend on the SAC checkpoint, only
on the system config (30 dBi / 75 W / nadir), which is unchanged from the
existing 3GPP nadir MMSE gzip already on disk (same reuse the benchmark
figure in my_plotting.py already relies on for the old-system MMSE curve).
"""
import os
...
detlam_training_names = [
    'EE_dinkelbach_adaptive_detlam512_N16K3_satg30_p75_eta0.6_rawpow',
    'EE_dinkelbach_adaptive_aod0.05_detlam512_N16K3_satg30_p75_eta0.6_rawpow',
    'EE_dinkelbach_adaptive_detlam512_N16K3_satg30_p75_eta0.6_rawpow_fair1.5',
    'EE_dinkelbach_adaptive_aod0.05_detlam512_N16K3_satg30_p75_eta0.6_rawpow_fair1.5',
]

for training_name in detlam_training_names:
    config = Config()
