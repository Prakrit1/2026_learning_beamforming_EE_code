...
ema0.3+warmstart detlam retrains (2026-07-30 session, jobs 155291-94),
...
every curve shares the exact same channel realizations, matching
beampattern_detlam.py's convention. Evaluated at CSIT error=0.05.

Compare printed coverage/offset numbers directly against the pre-ema
detlam session's findings (EE_beampattern_detlam_stats.gzip, from job
154995): pre-ema detlam checkpoints had coverage ~0.466-0.478 / offset
1.85-2.71 deg, notably worse than pre-fix's 0.601 / 1.28 deg -- this run
tests whether ema+warmstart closed that gap.
"""
import matplotlib
matplotlib.use('Agg')
...
TRAINING_NAME = 'EE_beampattern_detlam_emaws_N16K3'
...
MODEL_KEYS = ['prefix_nadir', 'ema_ws_aod0.0', 'ema_ws_aod0.05', 'ema_ws_aod0.0_fair1.5', 'ema_ws_aod0.05_fair1.5']
...
if __name__ == '__main__':
    ...
    if not PLOT_ONLY:
        ...
        for label, path in model_paths.items():
            print(f'[beampattern_detlam_ema_warmstart] {label} -> {path}')
    ...
    stats_out = Path(cfg.output_metrics_path, 'EE_beampattern_detlam_emaws_stats.gzip')
    with gzip.open(stats_out, 'wb') as file:
        pickle.dump(all_stats, file=file)
    print(f'\nSaved: {stats_out}')

    # ---- direct comparison against the pre-ema session's numbers -----------
    old_stats_path = Path(cfg.output_metrics_path, 'EE_beampattern_detlam_stats.gzip')
    if old_stats_path.exists():
        with gzip.open(old_stats_path, 'rb') as file:
            old_stats = pickle.load(file)
        print('\n=== Pre-ema vs. ema+warmstart: mean coverage / mean |offset| ===')
        pairs = [
        ...
        for old_key, new_key in pairs:
            if old_key not in old_stats or new_key not in all_stats:
                continue
            old_cov = np.mean(old_stats[old_key]['coverage'])
            new_cov = np.mean(all_stats[new_key]['coverage'])
            old_off = np.mean(np.abs(old_stats[old_key]['offset_deg']))
            new_off = np.mean(np.abs(all_stats[new_key]['offset_deg']))
            print(f'{old_key:24s}  coverage {old_cov:.3f} -> {new_cov:.3f}   '
                  f'offset {old_off:.2f} deg -> {new_off:.2f} deg')
    else:
        print(f'\n[note] old stats file {old_stats_path} not found, skipping before/after comparison')
    ...
    colors = {
        ...
        'ema_ws_aod0.0': plot_cfg.cp2['green'],
        'ema_ws_aod0.05': plot_cfg.cp2['blue'],
        'ema_ws_aod0.0_fair1.5': plot_cfg.cp3['orange1'],
        'ema_ws_aod0.05_fair1.5': plot_cfg.cp3['purple1'],
    }
    line_styles = {
        'mmse': 'dotted',
        'prefix_nadir': 'dashed',
        'ema_ws_aod0.0': 'solid',
        'ema_ws_aod0.05': 'solid',
        'ema_ws_aod0.0_fair1.5': 'dashdot',
        'ema_ws_aod0.05_fair1.5': (0, (3, 1, 1, 1)),
    }
    marker_styles = {
        'mmse': 'x',
        'prefix_nadir': '*',
        ...
        'ema_ws_aod0.05_fair1.5': 'd',
    }
    labels = {
        'mmse': 'MMSE',
        'prefix_nadir': 'SAC (pre-fix, rollout-lambda)',
        'ema_ws_aod0.0': 'SAC (detlam, aod0.0, ema+ws)',
        'ema_ws_aod0.05': 'SAC (detlam, aod0.05, ema+ws)',
        'ema_ws_aod0.0_fair1.5': 'SAC (detlam, aod0.0, fair1.5, ema+ws)',
        'ema_ws_aod0.05_fair1.5': 'SAC (detlam, aod0.05, fair1.5, ema+ws)',
    }
    ...
    out = Path(pdf_path, f'beampattern_detlam_emaws_realization{REALIZATION_TO_PLOT}.pdf')
