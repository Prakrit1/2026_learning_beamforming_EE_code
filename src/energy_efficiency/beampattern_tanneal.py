"""
Beampattern generation + beamwidth/coverage/offset quantification for the
target_entropy-annealing checkpoint (job 155683, 2026-07-31), alongside MMSE,
the pre-fix (rollout-lambda, non-annealed) SAC nadir checkpoint, and all four
detlam (deterministic-lambda, 2026-07-29) checkpoints -- all nadir geometry,
3GPP Set-1 defaults (30 dBi / 75 W). Direct extension of beampattern_detlam.py
with one more precoder (tanneal) so the aiming-accuracy corroboration used
there (coverage/offset) can be judged for entropy-annealing the same way.

All SEVEN precoders are generated from ONE generate_beampatterns() call so
every curve shares the exact same channel realizations. Evaluated at CSIT
error=0.05, matching every other beampattern_*.py script in this directory.
"""
import matplotlib
matplotlib.use('Agg')
...
TRAINING_NAME = 'EE_beampattern_tanneal_N16K3'
...
MODEL_KEYS = [
    'prefix_nadir', 'detlam_aod0.0', 'detlam_aod0.05', 'detlam_aod0.0_fair1.5',
    'detlam_aod0.05_fair1.5', 'tanneal_aod0.05',
]
...
CHECKPOINT_TRAINING_NAMES = {
    ...
    'tanneal_aod0.05': 'EE_dinkelbach_adaptive_aod0.05_lwin5000_N16K3_satg30_p75_eta0.6_tanneal1to0_rawpow',
}
...
if __name__ == '__main__':
    ...
    if not PLOT_ONLY:
        ...
        for label, path in model_paths.items():
            print(f'[beampattern_tanneal] {label} -> {path}')
    ...
    stats_out = Path(cfg.output_metrics_path, 'EE_beampattern_tanneal_stats.gzip')
    with gzip.open(stats_out, 'wb') as file:
        pickle.dump(all_stats, file=file)
    print(f'\nSaved: {stats_out}')

    # ---- diff against the existing detlam-set stats, if available --------
    detlam_stats_path = Path(cfg.output_metrics_path, 'EE_beampattern_detlam_stats.gzip')
    if detlam_stats_path.exists():
        with gzip.open(detlam_stats_path, 'rb') as file:
            detlam_stats = pickle.load(file)
        print('\n=== Coverage/offset vs. existing detlam-set stats (2026-07-29 fix) ===')
        for model in MODEL_KEYS:
            if model not in detlam_stats or not detlam_stats[model]['coverage']:
                continue
            old_cov = np.mean(detlam_stats[model]['coverage'])
            new_cov = np.mean(all_stats[model]['coverage'])
            old_off = np.mean(np.abs(detlam_stats[model]['offset_deg']))
            new_off = np.mean(np.abs(all_stats[model]['offset_deg']))
            print(f'  {model:<22} coverage {old_cov:.3f} -> {new_cov:.3f}   '
                  f'offset {old_off:.2f} -> {new_off:.2f} deg')
    ...
    out = Path(pdf_path, f'beampattern_tanneal_realization{REALIZATION_TO_PLOT}.pdf')
