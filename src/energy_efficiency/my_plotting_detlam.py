"""Plot error sweep (sum rate): the four completed detlam (deterministic-
lambda) checkpoints from the 2026-07-29 lambda/over-transmission fix vs.
MMSE and the pre-fix (rollout-lambda) 3GPP nadir checkpoint -- to see the
effect of the fix on rate. Run my_evaluation_detlam.py first if the
detlam gzips are missing.
"""
import matplotlib
matplotlib.use('Agg')
...
if __name__ == '__main__':
    ...
    # MMSE + pre-fix (rollout-lambda) SAC nadir: existing gzips, not re-run
    # (same system, unaffected by the lambda-mode change).
    rollout_nadir_folder = 'EE_dinkelbach_adaptive_aod0.05_lwin5000_N16K3_satg30_p75_eta0.6_rawpow'
    # New detlam checkpoints (2026-07-29 fix), aod0.025 variants excluded
    # (diverged to NaN mid-training, not evaluated).
    ...
    markerstyles = ['v', 'x', 'o', 's', '^', 'd']
    linestyles = ['-', '--', '-', '-', '--', '--']

    plot_error_sweep_testing_graph(
        paths=data_paths,
        metric='sumrate',
        name='error_sweep_detlam_vs_prefix',
        width=plot_width,
        height=plot_height,
        legend=legend,
        colors=colors,
    ...
    plt.savefig(
        Path(plot_cfg.plots_parent_path, 'pdf', 'error_sweep_detlam_vs_prefix_sumrate.pdf'),
        bbox_inches='tight', dpi=800, transparent=True,
    )
    print('Saved: error_sweep_detlam_vs_prefix_sumrate.pdf')
