...
rate_power_detlam.py) for the target_entropy-annealing checkpoint (job
155683, 2026-07-31). aod=0.05, nadir, 3GPP Set-1 defaults (30 dBi / 75 W),
lwin5000 lambda mode (NOT detlam) -- only new variable vs. the pre-fix
checkpoint is entropy annealing.

MMSE and the pre-fix (rollout-lambda, non-annealed) SAC nadir curve are NOT
re-simulated: loaded from rate_power_3gpp_pilots.gzip. The four detlam
checkpoints are ALSO not re-simulated: loaded from rate_power_detlam.gzip
(produced by rate_power_detlam.py) -- this is the direct three-way
comparison (pre-fix vs. detlam vs. entropy-anneal) the 2026-07-31 pilots
were launched to enable.
"""
import matplotlib
matplotlib.use('Agg')
...
COLORS = {
    'prefix_nadir': '#888888',
    'mmse_nadir': '#caa023',
    'detlam_aod0.0': '#307b3b',
    'detlam_aod0.05': '#254796',
    'detlam_aod0.0_fair1.5': '#f7a600',
    'detlam_aod0.05_fair1.5': '#3b296a',
    'tanneal_aod0.05': '#d03b3b',
}

LABELS = {
    'prefix_nadir': 'SAC pre-fix (rollout-lambda, aod0.05)',
    'mmse_nadir': 'MMSE nadir',
    ...
    'tanneal_aod0.05': 'SAC entropy-anneal aod0.05',
}
...
if __name__ == '__main__':
    ...
    detlam_gzip = Path(cfg0.output_metrics_path, *DETLAM_GZIP_REL)
    with gzip.open(detlam_gzip, 'rb') as file:
        detlam_data = pickle.load(file)
    if not np.allclose(detlam_data['error_sweep_range'], error_sweep_range):
        raise ValueError(f'error grid mismatch between {detlam_gzip} and this sweep')

    results = {
        'prefix_nadir': dict(prefix_data['results']['3gpp_nadir'], label=LABELS['prefix_nadir']),
        'mmse_nadir': dict(prefix_data['results']['mmse_nadir'], label=LABELS['mmse_nadir']),
    }
    for key in ['detlam_aod0.0', 'detlam_aod0.05', 'detlam_aod0.0_fair1.5', 'detlam_aod0.05_fair1.5']:
        results[key] = dict(detlam_data['results'][key], label=LABELS[key])
    print(f'[reuse] loaded prefix_nadir + mmse_nadir from {prefix_gzip}, '
          f'four detlam checkpoints from {detlam_gzip} (none re-simulated)')

    out_path = Path(cfg0.output_metrics_path, 'EE_tanneal_rate_power_sweep')
    out_path.mkdir(parents=True, exist_ok=True)
    gzip_path = Path(out_path, 'rate_power_tanneal.gzip')

    cfg = make_config()
    cfg.config_learner.training_name = TANNEAL_TRAINING_NAME
    model_path = get_best_model_path(cfg.trained_models_path, TANNEAL_TRAINING_NAME)
    print(f'[tanneal_aod0.05] checkpoint: {model_path}')

    precoder_network, norm_factors = load_model(model_path)
    if norm_factors != {}:
        cfg.config_learner.get_state_args['norm_state'] = True
    results['tanneal_aod0.05'] = run_rate_power_sweep(
        cfg, LABELS['tanneal_aod0.05'],
        lambda c, um, sm: get_precoding_learned_clip_only(c, um, sm, norm_factors, precoder_network),
    )
    results['tanneal_aod0.05']['label'] = LABELS['tanneal_aod0.05']
    ...
    for key, data in results.items():
        ...
        print(f"{data['label']:35s}  mean power={mp:6.2f} W  "
    ...
    fig.suptitle('Entropy-anneal checkpoint (2026-07-31) vs. detlam (07-29), pre-fix SAC and MMSE, nadir 30 dBi / 75 W', fontsize=11.5, y=1.02)
    fig.tight_layout(rect=[0, 0, 1, 0.96])

    pdf_path = Path(plot_cfg.plots_parent_path, 'pdf')
    pdf_path.mkdir(parents=True, exist_ok=True)
    out = Path(pdf_path, 'rate_power_tanneal.pdf')
